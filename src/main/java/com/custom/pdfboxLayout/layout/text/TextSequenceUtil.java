...
import com.custom.pdfboxLayout.layout.elements.Dividable;
import com.custom.pdfboxLayout.layout.elements.Paragraph;
import com.custom.pdfboxLayout.layout.util.Pair;
import com.custom.pdfboxLayout.layout.util.WordBreakerFactory;
...
public class TextSequenceUtil {
	...
	for (TextFragment fragment : text) {
		...
		line.add((LayoutStyledText) fragment);
	    }
	}
	if (!line.isEmpty()) {
	    result.add(line);
     ...
     *             by pdfbox
     */
    public static Dividable.Divided divide(final TextSequence text, final float maxWidth,
	...
	if (splitIndex == 0) {
	    ...
	    LayoutStyledText newWord = null;
	    if (word instanceof LayoutStyledText) {
		newWord = ((LayoutStyledText) word).inheritAttributes(text
			.substring(splitIndex));
	    } else {
		newWord = new LayoutStyledText(text.substring(splitIndex),
	...
	if (text instanceof NewLine) {
	    ...
	    float leftMargin = 0;
	    float rightMargin = 0;
	    if (text instanceof LayoutStyledText && ((LayoutStyledText) text).hasMargin()) {
		leftMargin = ((LayoutStyledText) text).getLeftMargin();
		rightMargin = ((LayoutStyledText) text).getRightMargin();
	    }
	...
	if (toDeriveFrom instanceof LayoutStyledText) {
	    return ((LayoutStyledText) toDeriveFrom).inheritAttributes(text,
		    leftMargin, rightMargin);
	}
	return new LayoutStyledText(text, toDeriveFrom.getFontDescriptor(),
	...
	float rightMargin = 0;
	if (word instanceof LayoutStyledText) {
	    LayoutStyledText layoutStyledText = (LayoutStyledText) word;
	    leftMargin = layoutStyledText.getLeftMargin();
	    rightMargin = layoutStyledText.getRightMargin();
	}
