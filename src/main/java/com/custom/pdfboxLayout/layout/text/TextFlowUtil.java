...
import com.custom.pdfboxLayout.layout.text.annotations.Annotation;
import com.custom.pdfboxLayout.layout.text.annotations.AnnotationCharacters;
...
public class TextFlowUtil {
	...
	for (final CharSequence fragment : parts) {
		...
		if (fragment instanceof ControlCharacters.ColorControlCharacter) {
		    color = ((ControlCharacters.ColorControlCharacter
					) fragment).getColor();
		}
		...
		if (annotationMap.isEmpty()) {
		    LayoutStyledText layoutStyledText = new LayoutStyledText(fragment.toString(),
			    currentFontSize, font, color, baselineOffset);
		    result.add(layoutStyledText);
		} else {
		    AnnotatedLayoutStyledText styledText = new AnnotatedLayoutStyledText(
